            ...
            <form class="form-horizontal form-label-left input_mask">
                <div class="row">
                    <div class="col-md-2 col-xs-12 form-group">
                        <input type="text" class="form-control" name="id" placeholder="Mã Bàn..." value="{{ Request::get('sochongoi') }}">
                    </div>

                    <div class="col-md-2 col-xs-12 form-group">
                        <form  method="POST" class="form-horizontal form-label-left">
                            <button href="ban" style="width: 100%" type="submit" class="btn btn-success btn-search"><i class="fa fa-search"></i> {{__('search')}}</button>
                        </form>
                    </div>
                    ...
                    <div class="col-md-2 col-xs-12 form-group">
                        <a href="themban" style="width: 100%" class="btn btn-primary btn-search"><i class="fa fa-plus"></i> {{__('Thêm Tài Khoản')}}</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="row">
            <div class="col-lg-12">
                    <div class="main-card mb-3 card">
                        <div class="card-body"><h5 class="card-title">Bàn</h5><a href=""></a>
                                ...
                                <table class="mb-0 table">
                                    ...
                                    @foreach($data as $key => $value)
                                        <tr style="color: #000000bf;" data-target="{{$value->id}}">
                                            ...
                                            <td>
                                                @if($value->loaiban == LOAI_BAN_VIP) {{ "VIP" }} @else {{ "THƯỜNG" }} @endif
                                            </td>
                                            <td>{{ $value->mota }}</td>
                                            
                                            <td style="display: flex;">
                                                <a href="ban/destroy/{{$value->id}}" class="mb-2 mr-2 btn btn-danger"><i class="pe-7s-trash" style="font-size: 20px;"> </i></a>
                                                <a data-toggle="modal" data-target=".bd-example-modal-lg" class="mb-2 mr-2 btn btn-info btnsua"><i class="pe-7s-pen" style="font-size: 20px;"> </i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
...
@section('modal')
<form action="{{route("ban.sua")}}" method="POST" accept-charset="utf-8">
    @csrf
